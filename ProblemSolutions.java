...
public class ProblemSolutions {
    ...
    public static void selectionSort(int[] values, boolean ascending ) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            // swap min or max with first element
            int temp = values[i];
    ...
    private void mergeDivisbleByKFirst(int arr[], int k, int left, int mid, int right)
    {
        ...
        // hold data in subarrays
        int[] temp = new int[right - left + 1];
        int index = 0;

        // merge divisble k elements first
        int i = 0, j = 0;
        while (i < n1 && j < n2) {
        ...
        // merge remaining elements in ascending order
        while (i < n1 && j < n2) {
            if (leftArr[i] % k != 0 && rightArr[j] % k != 0) {
                temp[index++] = (leftArr[i] <= rightArr[j]) ? leftArr[i++] : rightArr[j++];
            } else if (i < n1 && leftArr[i] % k != 0) {
                temp[index++] = leftArr[i++];
            } else if (j < n2 && rightArr[j] % k != 0) {
                temp[index++] = rightArr[j++];
            }
        }

        // add any leftover elements from left subarray
        while (i < n1) {
            temp[index++] = leftArr[i++];
        }
    ...
    public static boolean asteroidsDestroyed(int mass, int[] asteroids) {
        ...
        for (int asteroid : asteroids) {
            if (currentMass >= asteroid) {
                currentMass += asteroid; // asteroid's mass
            } else {
                return false; // planet cannot destroy asteroid
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(asteroidsDestroyed(mass1, asteroids1)); // Output: true

        // Example 2
        int mass2 = 5;
        int[] asteroids2 = {4, 9, 23, 4};
        System.out.println(asteroidsDestroyed(mass2, asteroids2)); // Output: false
    ...
    public static int numRescueSleds(int[] people, int limit) {
        ...
        while (i <= j) {
            ...
            if (people[i] + people[j] <= limit) {
                i++; // Move the lightest pointer
            }
            // Move the heaviest pointer
            j--;
            // Count a sled for the pair or single person
            sleds++;
        }
